...
public class DefaultTeam {
    ...
    for (int i = 0; i < points.size(); i++) {
      for (int j = i + 1; j < points.size(); j++) {
        for (int k = j + 1; k < points.size(); k++) {
          ...
          // r="+Math.sqrt(cRadiusSquared));resX=cX;resY=cY;resRadiusSquared=cRadiusSquared;}
        }
      }
    }
  ...
  // Fonction d'appel de la fonction de calcul de base du cercle minimum optimisée
